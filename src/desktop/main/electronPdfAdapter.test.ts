...
describe('Electron PDF adapter', () => {
  ...
  it('forwards fixed PDF operation through trusted main frame while bridge exposes only typed operations', async () => {
    ...
    expect(Object.keys(bridge).sort()).toEqual(['compareEngineeringRuns', 'executeEngineeringAnalysis', 'exportEngineeringPdf', 'getLicenseStatus', 'importLicense', 'importVerificationEvidence', 'listEngineeringRuns', 'listVerificationEvidencePackages', 'loadEngineeringRun', 'loadVerificationEvidencePackage']); expect(Object.isFrozen(bridge)).toBe(true);
  ...
  it('exposes only the typed API, never raw ipcRenderer', async () => {
    await import('../preload/index');
    expect(mock.expose).toHaveBeenCalledWith('tolue', {
      executeEngineeringAnalysis: expect.any(Function),
      loadEngineeringRun: expect.any(Function),
      listEngineeringRuns: expect.any(Function),
      compareEngineeringRuns: expect.any(Function),
      exportEngineeringPdf: expect.any(Function),
      getLicenseStatus: expect.any(Function),
      importLicense: expect.any(Function),
      importVerificationEvidence: expect.any(Function),
      listVerificationEvidencePackages: expect.any(Function),
      loadVerificationEvidencePackage: expect.any(Function),
    });
  });
});
