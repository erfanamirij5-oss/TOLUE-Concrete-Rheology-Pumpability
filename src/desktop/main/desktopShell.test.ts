...
const m = vi.hoisted(() => ({
  ...
  on: vi.fn(), once: vi.fn(), open: vi.fn(), registerPdf: vi.fn(), registerAnalysis: vi.fn(), registerRunLoad: vi.fn(), registerVerification: vi.fn(), registerLicense: vi.fn(), disposePdf: vi.fn(), disposeAnalysis: vi.fn(), disposeRunLoad: vi.fn(), disposeVerification: vi.fn(), disposeLicense: vi.fn(),
  request: vi.fn(), check: vi.fn(), filter: vi.fn(), protocol: vi.fn(), sessionOn: vi.fn(), error: vi.fn(), read: vi.fn(),
  bootstrapPersistence: vi.fn(), closePersistence: vi.fn(), saveRun: vi.fn(), findRun: vi.fn(), saveVerification: vi.fn(), findVerification: vi.fn(), listVerification: vi.fn(),
}));
vi.mock('electron', () => ({
...
vi.mock('./electronVerificationEvidenceAdapter', () => ({ registerVerificationEvidenceIpc: m.registerVerification }));
vi.mock('./electronLicenseAdapter', () => ({ registerLicenseIpc: m.registerLicense }));
...
beforeEach(() => {
  ...
  m.bootstrapPersistence.mockReturnValue({
    databasePath: '/tolue/db',
    migration: { fromVersion: 0, toVersion: 2, appliedVersions: [1,2], method: 'tolue-persistence-migration-v2' },
    engineeringRuns: { save: m.saveRun, findByRunId: m.findRun },
    verificationEvidencePackages: { save: m.saveVerification, findByPackageId: m.findVerification, listHistory: m.listVerification },
    close: m.closePersistence,
  });
  m.load.mockResolvedValue(undefined); m.registerPdf.mockReturnValue(m.disposePdf); m.registerAnalysis.mockReturnValue(m.disposeAnalysis); m.registerRunLoad.mockReturnValue(m.disposeRunLoad); m.registerVerification.mockReturnValue(m.disposeVerification); m.registerLicense.mockReturnValue(m.disposeLicense);
});
describe('secure desktop shell', () => {
  it('serves only exact allowlisted assets with restrictive CSP and Persian RTL', async () => {
    const page=desktopResponse(DESKTOP_URL,'GET',rendererJs); expect(page.status).toBe(200); expect(page.headers.get('Content-Security-Policy')).toContain("script-src 'self'"); const html=await page.text(); expect(html).toContain('lang="fa" dir="rtl"'); expect(html).toContain(`src="${DESKTOP_RENDERER_URL}"`); expect(html).toContain('#app{display:block;width:100%;height:100%;max-width:none;margin:0;padding:0;overflow:hidden'); expect(html).not.toContain('max-width:900px'); expect(html).not.toContain('margin:12vh auto'); const script=desktopResponse(DESKTOP_RENDERER_URL,'GET',rendererJs); expect(script.status).toBe(200); expect(await script.text()).toBe(rendererJs); for(const url of ['file:///etc/passwd','https://evil.test','tolue://desktop/../secret',DESKTOP_URL+'?path=x',DESKTOP_RENDERER_URL+'?x=1'])expect(desktopResponse(url,'GET',rendererJs).status).toBe(404); expect(desktopResponse(DESKTOP_URL,'POST',rendererJs).status).toBe(404);
  });
  ...
  it('waits for readiness, bootstraps persistence, and installs isolated window', async()=>{startDesktopShell(preload,renderer);await flush();expect(m.read).toHaveBeenCalledWith(renderer,'utf8');expect(m.sandbox).toHaveBeenCalledOnce();expect(m.options).toHaveBeenCalledWith(expect.objectContaining({show:false,webPreferences:expect.objectContaining({preload,sandbox:true,contextIsolation:true,nodeIntegration:false,webviewTag:false})}));expect(m.registerPdf).toHaveBeenCalledWith(expect.anything(),DESKTOP_URL);expect(m.registerAnalysis).toHaveBeenCalled();expect(m.registerRunLoad).toHaveBeenCalled();expect(m.registerVerification).toHaveBeenCalledWith(expect.objectContaining({trustedDocumentUrl:DESKTOP_URL,repository:expect.anything(),nowIso:expect.any(Function)}));expect(m.registerLicense).toHaveBeenCalledWith(expect.objectContaining({trustedDocumentUrl:DESKTOP_URL,userDataPath:expect.stringContaining(DESKTOP_USER_DATA_DIRECTORY)}));expect(m.load).toHaveBeenCalledWith(DESKTOP_URL);expect(m.show).toHaveBeenCalledOnce();expect(m.open.mock.calls[0]![0]()).toEqual({action:'deny'});const cb=vi.fn();m.filter.mock.calls[0]![0]({url:'https://evil.test'},cb);expect(cb).toHaveBeenCalledWith({cancel:true});m.request.mock.calls[0]![0](null,'camera',cb);expect(cb).toHaveBeenLastCalledWith(false);m.once.mock.calls.find(c=>c[0]==='closed')![1]();expect(m.disposeLicense).toHaveBeenCalledOnce();expect(m.disposeVerification).toHaveBeenCalledOnce();expect(m.disposeRunLoad).toHaveBeenCalledOnce();expect(m.disposePdf).toHaveBeenCalledOnce();expect(m.disposeAnalysis).toHaveBeenCalledOnce();m.appOn.mock.calls.find(c=>c[0]==='before-quit')![1]();expect(m.closePersistence).toHaveBeenCalledOnce();});
