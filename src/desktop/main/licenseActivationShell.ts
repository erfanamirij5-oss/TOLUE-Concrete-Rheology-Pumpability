...
export function startLicenseActivationShell(preloadPath: string, userDataPath: string): void {
  ...
  const open = async (): Promise<void> => {
    ...
    const disposeLicense = registerLicenseIpc({ owner: win, trustedDocumentUrl: ACTIVATION_URL, userDataPath, resourcesPath: process.resourcesPath, nowIso: () => new Date().toISOString() });
