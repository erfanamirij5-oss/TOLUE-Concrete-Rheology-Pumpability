...
export function renderVerificationPackageHistoryView(
  ...
  const title=document.createElement('strong');title.textContent='Recent Verification Packages';title.style.direction='ltr';
  ...
  if(!items.length){const empty=document.createElement('div');empty.textContent='هنوز Verification Package ذخیره‌شده‌ای وجود ندارد.';empty.style.color=TOLUE_DESIGN_TOKENS.color.textMuted;empty.style.padding='8px';root.appendChild(empty);return;}
  const table=document.createElement('table');Object.assign(table.style,{width:'100%',borderCollapse:'collapse',fontSize:TOLUE_DESIGN_TOKENS.typography.fontSizeSm,direction:'ltr',textAlign:'left'});
  const trh=document.createElement('tr');['Package ID','Generated','Imported','Entries','Generated by','Purpose',''].forEach(text=>{const th=document.createElement('th');th.textContent=text;th.style.padding='7px';th.style.borderBottom=`1px solid ${TOLUE_DESIGN_TOKENS.color.borderStrong}`;trh.appendChild(th);});table.appendChild(trh);
