...
export function renderLubricationLayerQualificationView(root:HTMLElement,input:Readonly<SimulationRunInput>,actions:Readonly<LubricationLayerQualificationViewActions>):void{
  ...
  const heading=document.createElement('strong');heading.textContent='Lubrication Layer Qualification';
  const note=document.createElement('p');note.textContent='ضخامت و رئولوژی LL فقط وقتی qualified هستند که mode و provenance آن‌ها صریحاً تعریف و به همان Entity IDها متصل شوند. هیچ binding خودکاری بدون رکورد موجود انجام نمی‌شود.';note.style.color=TOLUE_DESIGN_TOKENS.color.textMuted;note.style.fontSize=TOLUE_DESIGN_TOKENS.typography.fontSizeXs;
  ...
  const mode=document.createElement('select');for(const item of MODES){const option=document.createElement('option');option.value=item;option.textContent=item;mode.appendChild(option);}mode.value=q?.mode??'MEASURED_TRIBOLOGY';mode.style.padding='7px';mode.style.fontFamily='inherit';
  const setMode=document.createElement('button');setMode.type='button';setMode.textContent=q?'تغییر Mode':'تعریف Qualification';setMode.addEventListener('click',()=>actions.updateInput(setLubricationLayerQualificationModeDraft(input,mode.value as LubricationLayerQualificationMode)));
  ...
  if(q){
    ...
    const fields:readonly [string,string,string][]=[['rheologyEvidenceEntityId','Rheology Evidence Entity ID',q.rheologyEvidenceEntityId??''],['thicknessEvidenceEntityId','Thickness Evidence Entity ID',q.thicknessEvidenceEntityId??''],['methodId','Method ID',q.methodId??''],['referenceId','Reference ID',q.referenceId??''],['note','Note',q.note??'']];
    ...
    const bind=document.createElement('button');bind.type='button';bind.textContent='اتصال به Provenance فعلی LL';bind.style.gridColumn='1 / -1';bind.addEventListener('click',()=>{try{actions.updateInput(bindLubricationLayerQualificationToCurrentProvenance(input));}catch(error){bind.title=error instanceof Error?error.message:'LLQ bind failed';}});
    const remove=document.createElement('button');remove.type='button';remove.textContent='حذف Qualification';remove.style.gridColumn='1 / -1';remove.addEventListener('click',()=>actions.updateInput(removeLubricationLayerQualificationDraft(input)));
  ...
  const legend=document.createElement('legend');legend.textContent='Thickness Provenance';provBox.appendChild(legend);
  ...
  const entityKind=document.createElement('select');ENTITY_KINDS.forEach(v=>{const o=document.createElement('option');o.value=v;o.textContent=v;entityKind.appendChild(o);});entityKind.value=prov?.evidence.entityKind??'measurement_result';
  const activityKind=document.createElement('select');ACTIVITY_KINDS.forEach(v=>{const o=document.createElement('option');o.value=v;o.textContent=v;activityKind.appendChild(o);});activityKind.value=activity?.kind??'measurement';
  pgrid.append(wrap('Entity ID',entityInput),wrap('Entity Kind',entityKind),wrap('Activity ID',activityInput),wrap('Activity Kind',activityKind),wrap('Method ID',methodInput),wrap('Equipment ID',equipmentInput),wrap('Equipment Label',equipmentLabelInput),wrap('Source Document ID',sourceDocInput),wrap('Reference ID',referenceInput),wrap('Note',noteInput));
  const save=document.createElement('button');save.type='button';save.textContent=prov?'به‌روزرسانی Thickness Provenance':'ثبت Thickness Provenance';save.style.gridColumn='1 / -1';save.addEventListener('click',()=>{try{actions.updateInput(setLubricationLayerThicknessProvenanceDraft(input,{entityId:entityInput.value,entityKind:entityKind.value as ProvenanceEntityKind,activityId:activityInput.value,activityKind:activityKind.value as ProvenanceActivityKind,methodId:methodInput.value,equipmentId:equipmentInput.value,equipmentLabel:equipmentLabelInput.value,sourceDocumentId:sourceDocInput.value,referenceId:referenceInput.value,note:noteInput.value}));}catch(error){save.title=error instanceof Error?error.message:'Thickness provenance failed';}});
  pgrid.appendChild(save);
  if(prov){const remove=document.createElement('button');remove.type='button';remove.textContent='حذف Thickness Provenance';remove.style.gridColumn='1 / -1';remove.addEventListener('click',()=>actions.updateInput(removeLubricationLayerThicknessProvenanceDraft(input)));pgrid.appendChild(remove);}
