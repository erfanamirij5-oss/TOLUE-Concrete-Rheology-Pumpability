...
export interface DiagnosticSeverityUx {
...
export function diagnosticKindLabel(kind: DiagnosticFinding['kind']): string {
  const labels: Readonly<Record<DiagnosticFinding['kind'], string>> = Object.freeze({
    INCOMPLETE_ANALYSIS: 'تحلیل ناقص',
    INSUFFICIENT_DATA: 'داده ناکافی',
    PUMP_PRESSURE_INSUFFICIENT: 'فشار پمپ ناکافی',
    PUMP_PRESSURE_ADEQUATE: 'فشار پمپ کافی',
    STABILITY_UNACCEPTABLE: 'پایداری نامطلوب',
    BLOCKAGE_UNACCEPTABLE: 'ریسک انسداد نامطلوب',
    PUMPABILITY_PROJECT_QUALIFIED: 'پمپ‌پذیری تأییدشده برای پروژه',
    PUMPABILITY_PARTIALLY_QUALIFIED: 'پمپ‌پذیری با تأیید جزئی',
  });
  return labels[kind];
}
...
export function diagnosticBasisLabel(basis: DiagnosticFinding['basis']): string {
  ...
  return 'شواهد واجدصلاحیت پروژه';
}

export function diagnosticValidationLabel(status: EngineeringValidationStatus): string {
  const labels: Readonly<Record<EngineeringValidationStatus, string>> = Object.freeze({
    verified: 'اعتبارسنجی‌شده',
