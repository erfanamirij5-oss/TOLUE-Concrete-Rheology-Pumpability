...
/** Renderer-side draft editing only. No engineering defaults or acceptance criteria are inferred here. */
...
export function removePumpCapabilityDraft(input: Readonly<SimulationRunInput>): Readonly<SimulationRunInput> {
  const next = structuredClone(input) as SimulationRunInput;
  delete next.pumpCapability;
  return Object.freeze(next);
}

export function updatePumpCapabilityProvenanceDraft(input: Readonly<SimulationRunInput>, provenance: PumpCapabilityProvenance): Readonly<SimulationRunInput> {
  if (!input.pumpCapability) throw new Error('ENGINEERING-INPUT-DRAFT-PUMP-001');
  const next = structuredClone(input) as SimulationRunInput;
  next.pumpCapability!.provenance = provenance;
  return Object.freeze(next);
}

export function updatePumpOperatingEnvelopeDraft(input: Readonly<SimulationRunInput>, field: PumpOperatingEnvelopeField, value: string): Readonly<SimulationRunInput> {
  if (!input.pumpCapability) throw new Error('ENGINEERING-INPUT-DRAFT-PUMP-001');
  const next = structuredClone(input) as SimulationRunInput;
  const envelope = next.pumpCapability!.operatingEnvelope ?? {
    manufacturer: '', model: '', configurationRevision: '', sourceDocumentId: '', sourceDocumentRevision: '',
  };
  if (field === 'sourceHash') {
    if (value.trim()) envelope.sourceHash = value.trim(); else delete envelope.sourceHash;
  } else {
    envelope[field] = value;
  }
  next.pumpCapability!.operatingEnvelope = envelope;
  return Object.freeze(next);
}

export function addPumpCapabilityPointDraft(input: Readonly<SimulationRunInput>, flowRateM3s: number, availableConcretePressurePa: number): Readonly<SimulationRunInput> {
  finite(flowRateM3s); finite(availableConcretePressurePa);
  if (!input.pumpCapability) throw new Error('ENGINEERING-INPUT-DRAFT-PUMP-001');
  if (flowRateM3s < 0) throw new Error('ENGINEERING-INPUT-DRAFT-PUMP-FLOW-001');
  if (availableConcretePressurePa < 0) throw new Error('ENGINEERING-INPUT-DRAFT-PUMP-PRESSURE-001');
  const next = structuredClone(input) as SimulationRunInput;
  const curve = next.pumpCapability!.capabilityCurve;
  if (curve.some(point => point.flowRateM3s === flowRateM3s)) throw new Error('ENGINEERING-INPUT-DRAFT-PUMP-DUPLICATE-FLOW-001');
  curve.push({ flowRateM3s, availableConcretePressurePa });
  curve.sort((a, b) => a.flowRateM3s - b.flowRateM3s);
  return Object.freeze(next);
}

export function removePumpCapabilityPointDraft(input: Readonly<SimulationRunInput>, pointIndex: number): Readonly<SimulationRunInput> {
  if (!input.pumpCapability) throw new Error('ENGINEERING-INPUT-DRAFT-PUMP-001');
  if (!input.pumpCapability.capabilityCurve[pointIndex]) throw new Error('ENGINEERING-INPUT-DRAFT-PUMP-POINT-001');
  const next = structuredClone(input) as SimulationRunInput;
  next.pumpCapability!.capabilityCurve.splice(pointIndex, 1);
  return Object.freeze(next);
}

export function updatePumpCapabilityPointDraft(input: Readonly<SimulationRunInput>, pointIndex: number, field: PumpCapabilityPointField, value: number): Readonly<SimulationRunInput> {
  finite(value);
  if (!input.pumpCapability) throw new Error('ENGINEERING-INPUT-DRAFT-PUMP-001');
