...
export function renderApplicationShell(root:HTMLElement,dataFlow:Readonly<ApplicationDataFlowState>=createApplicationDataFlowState(),actions?:Readonly<ApplicationShellActions>):void{
  ...
  const updateSessionInput=(input:Readonly<SimulationRunInput>)=>{sessionState=setAnalysisInput(sessionState,input);selectedSegmentId=null;renderAll();};
  ...
  function renderTree(){treeBody.replaceChildren();treeButton('▾  پروژه', 'project');treeButton('مصالح','materials',1);treeButton('رئولوژی','rheology',1);treeButton('پمپ','pump',1);treeButton('▾  خط لوله','pipeline',1);const segments=sessionState.analysis?.visualization3d?.segments??[];for(const segment of segments){const b=document.createElement('button');b.type='button';b.textContent=`${segment.id} · ${segment.kind}`;b.style.width='100%';b.style.textAlign='left';b.style.direction='ltr';b.style.paddingInlineStart='38px';buttonStyle(b,selectedSegmentId===segment.id);b.addEventListener('click',()=>{selectedSegmentId=segment.id;inspectorMode='segment';renderTree();renderInspector();renderViewport();});treeBody.appendChild(b);}treeButton('شواهد','evidence',1);if(!segments.length){const empty=document.createElement('small');empty.textContent='پس از تحلیل، اشیای مسیر اینجا ظاهر می‌شوند.';Object.assign(empty.style,{display:'block',padding:'8px 12px',color:TOLUE_DESIGN_TOKENS.color.textMuted});treeBody.appendChild(empty);}}

  function renderInspector(){inspectorBody.replaceChildren();const analysis=sessionState.analysis;if(inspectorMode==='project')renderProjectView(inspectorBody);else if(inspectorMode==='materials')renderMaterialsView(inspectorBody);else if(inspectorMode==='rheology')renderRheologyView(inspectorBody,undefined,analysis?.rheologyCurves??undefined,sessionState.input,sessionState.input?{updateInput:updateSessionInput}:undefined);else if(inspectorMode==='pipeline')renderPipelineView(inspectorBody,analysis?.pipeline??undefined,analysis?.pressureProfile??undefined,analysis?.pressureComposition??undefined,sessionState.input,sessionState.input?{updateInput:updateSessionInput}:undefined);else if(inspectorMode==='pump')renderPumpView(inspectorBody,analysis?.pump??undefined,sessionState.input,sessionState.input?{updateInput:updateSessionInput}:undefined);else if(inspectorMode==='evidence')renderEvidenceView(inspectorBody);else{const segment=analysis?.visualization3d?.segments.find(item=>item.id===selectedSegmentId);if(!segment){const p=document.createElement('p');p.textContent='هیچ جزء مهندسی انتخاب نشده است.';p.style.color=TOLUE_DESIGN_TOKENS.color.textMuted;inspectorBody.appendChild(p);return;}const h=document.createElement('h3');h.textContent=segment.id;h.style.direction='ltr';h.style.textAlign='left';const table=document.createElement('dl');table.style.display='grid';table.style.gridTemplateColumns='1fr 1fr';table.style.gap='7px 12px';const rows:[string,string][]=[['نوع',segment.kind],['Station start',segment.startStationM===null?'—':`${segment.startStationM} m`],['Station end',segment.endStationM===null?'—':`${segment.endStationM} m`],['Elevation start',`${segment.startElevationM} m`],['Elevation end',`${segment.endElevationM} m`],['Pipe radius',segment.pipeRadiusM===null?'—':`${segment.pipeRadiusM} m`],['ΔP',segment.totalPressureChangePa.value===null?'—':`${(segment.totalPressureChangePa.value/1e6).toFixed(3)} MPa`],['Q',segment.flowRateM3s.value===null?'—':`${segment.flowRateM3s.value.toFixed(4)} m³/s`],['Hydraulic',segment.hydraulicStatus]];for(const [k,v] of rows){const dt=document.createElement('dt');dt.textContent=k;dt.style.color=TOLUE_DESIGN_TOKENS.color.textMuted;const dd=document.createElement('dd');dd.textContent=v;dd.style.margin='0';dd.style.direction=/[A-Za-zΔ]/.test(k)?'ltr':'rtl';table.append(dt,dd);}inspectorBody.append(h,table);}}

  function renderViewport(){viewport.replaceChildren();renderVisualization3DView(viewport,sessionState.analysis?.visualization3d??undefined,{selectedSegmentId,onSelectSegment:(id)=>{selectedSegmentId=id;inspectorMode=id?'segment':'pipeline';renderTree();renderInspector();renderViewport();},compact:true});}
  ...
  function refreshTop(){const copy=sessionUxCopy(sessionState);saveState.textContent=sessionState.isStale?'نتایج قدیمی / ورودی تغییر کرده':copy.title;saveState.style.color=sessionState.isStale?TOLUE_DESIGN_TOKENS.color.statusWarning:TOLUE_DESIGN_TOKENS.color.textMuted;const ux=analysisExecutionUxState(sessionState,Boolean(actions));runTop.textContent=ux.label;runTop.disabled=!ux.enabled;runTop.setAttribute('aria-busy',ux.busy?'true':'false');projectContext.textContent=sessionState.activeRunId?`Run ${sessionState.activeRunId}`:'Engineering Workspace';projectContext.style.direction=sessionState.activeRunId?'ltr':'rtl';}
