...
import type { VerificationEvidenceImportIpcResponse } from '../ipc/verificationEvidenceIpc';
...
export function renderApplicationShell(root:HTMLElement,dataFlow:Readonly<ApplicationDataFlowState>=createApplicationDataFlowState(),actions?:Readonly<ApplicationShellActions>):void{
  ...
  const importVerificationEvidence=()=>{if(!actions||verificationImportBusy)return;verificationImportBusy=true;verificationImportError=null;renderBottom();void actions.importVerificationEvidence().then(response=>{if(response.status==='IMPORTED'&&response.evidencePackage){verificationPackage=response.evidencePackage;verificationImportError=null;}else if(response.status==='REJECTED'){verificationImportError=response.errorCode??'VERIFICATION-EVIDENCE-IMPORT-REJECTED';}}).catch(()=>{verificationImportError='VERIFICATION-EVIDENCE-IMPORT-TRANSPORT-001';}).finally(()=>{verificationImportBusy=false;renderBottom();});};
