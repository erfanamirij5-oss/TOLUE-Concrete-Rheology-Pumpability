...
describe('result center presentation boundary', () => {
  it('preserves Core result classification, validation, evidence, and null values', () => {
    const presentation = createResultCenterPresentation({
      ...
      results: [{ id: 'pipeline.requiredPressure', label: 'Required pressure', value: null, unit: 'Pa', resultClass: 'COMPOSITE_ENGINEERING_RESULT', methodId: 'pipeline-method', methodVersion: 'v1', referenceIds: [], standardEditionIds: [], applicability: 'project domain', assumptions: [], limitations: ['incomplete'], validationStatus: 'insufficient_data', evidenceStatus: 'BLOCKED', inputSnapshotHash: 'hash-1', sourceRunId: 'run-1', provenanceEntityIds: ['entity-1'], calibrationIds: ['cal-1'] }],
    });
  ...
  it('preserves the Core pumpability decision axes without reclassification', () => {
    const presentation = createResultCenterPresentation({
      ...
      runId: 'run-2', pressureFeasibility: 'PASS', stability: 'ACCEPTABLE', blockageRisk: 'OUT_OF_DOMAIN', stabilityBasis: 'PROJECT_QUALIFIED_EVIDENCE', blockageBasis: 'OUT_OF_DOMAIN_PROJECT_EVIDENCE', stabilityEvidence: null, blockageEvidence: null, riskScreening: null,
      status: 'PARTIALLY_QUALIFIED_ACCEPTABLE', sourceMethodIds: [], limitations: [], method: 'tolue-pumpability-decision-v3',
    });
