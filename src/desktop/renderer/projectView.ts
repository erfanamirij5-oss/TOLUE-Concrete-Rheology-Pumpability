...
function appendDraftIntegrityControl(card: HTMLElement, engineeringInput: Readonly<SimulationRunInput>, actions: Readonly<ProjectViewActions>): void {
  ...
  heading.textContent = placeholderActive ? 'Starter Draft Lock فعال است' : 'Starter Draft Lock تأیید شده است';
  box.appendChild(heading);

  const text = document.createElement('p');
  text.style.margin = '0';
  text.style.color = TOLUE_DESIGN_TOKENS.color.textMuted;
  text.style.fontSize = TOLUE_DESIGN_TOKENS.typography.fontSizeSm;
  text.textContent = placeholderActive
    ? 'مقادیر اولیه Draft فقط برای فعال بودن ویرایشگرها هستند و تا تأیید صریح شما تحلیل مهندسی BLOCKED می‌ماند. تغییر یک یا چند عدد به‌تنهایی این قفل را باز نمی‌کند.'
    : 'نشانگر Starter Placeholder از این Run حذف شده است. سایر قواعد Readiness، provenance، geometry، pump و LL همچنان مستقل اعمال می‌شوند.';
  box.appendChild(text);

  if (!placeholderActive) {
    const state = document.createElement('small');
    state.textContent = 'Integrity state: STARTER_VALUES_EXPLICITLY_ACKNOWLEDGED';
    state.style.direction = 'ltr';
    state.style.color = TOLUE_DESIGN_TOKENS.color.statusNominal;
  ...
  acknowledgement.style.alignItems = 'flex-start';
  acknowledgement.style.gap = TOLUE_DESIGN_TOKENS.spacing.sm;
  const checkbox = document.createElement('input');
  checkbox.type = 'checkbox';
  checkbox.dataset.draftIntegrityAcknowledgement = 'true';
  const labelText = document.createElement('span');
  labelText.textContent = 'تأیید می‌کنم مقادیر اولیه Draft را بررسی کرده‌ام و هر مقدار مهندسی مورد استفاده، عمداً با داده پروژه جایگزین یا توسط من تأیید شده است.';
  acknowledgement.append(checkbox, labelText);

  const confirm = document.createElement('button');
  confirm.type = 'button';
  confirm.textContent = 'تأیید داده‌های واقعی و باز کردن Starter Lock';
  confirm.disabled = true;
  confirm.style.fontFamily = 'inherit';
  confirm.style.padding = `${TOLUE_DESIGN_TOKENS.spacing.sm} ${TOLUE_DESIGN_TOKENS.spacing.md}`;
  confirm.style.border = `1px solid ${TOLUE_DESIGN_TOKENS.color.borderStrong}`;
  confirm.style.borderRadius = TOLUE_DESIGN_TOKENS.radius.sm;
  confirm.style.background = TOLUE_DESIGN_TOKENS.color.surfaceElevated;
  confirm.style.color = TOLUE_DESIGN_TOKENS.color.text;
  ...
  confirm.addEventListener('click', () => {
    try {
      const next = confirmStarterDraftInputs(engineeringInput, checkbox.checked);
      feedback.textContent = '';
      actions.updateInput(next);
    } catch (error) {
      feedback.textContent = error instanceof Error ? error.message : 'DRAFT-INTEGRITY-UNKNOWN';
    }
  });

  box.append(acknowledgement, confirm, feedback);
  card.appendChild(box);
}
