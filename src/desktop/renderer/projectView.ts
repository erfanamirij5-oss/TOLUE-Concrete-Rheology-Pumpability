...
function appendDraftIntegrityControl(card: HTMLElement, engineeringInput: Readonly<SimulationRunInput>, actions: Readonly<ProjectViewActions>): void {
  ...
  const headingText=document.createElement('strong');headingText.textContent=placeholderActive?'Starter Draft Lock فعال است':'Starter Draft Lock تأیید شده است';heading.append(statusDot,headingText);box.appendChild(heading);
  ...
  const confirm = document.createElement('button'); confirm.type = 'button'; confirm.textContent = 'تأیید داده‌های واقعی و باز کردن Starter Lock'; confirm.disabled = true; styleEngineeringButton(confirm,true);
...
export function renderProjectView(target: HTMLElement, engineeringInput?: Readonly<SimulationRunInput> | null, actions?: Readonly<ProjectViewActions>): void {
  ...
  appendEngineeringSectionHeader(card,'مشخصات پروژه','هویت، محل و کارفرمای پروژه در Snapshot همان Run ذخیره می‌شوند.','PROJECT');
