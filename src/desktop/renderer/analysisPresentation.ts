...
export interface EngineeringAnalysisPresentation {
  ...
  readonly pressureComposition: Readonly<PressureCompositionPresentation> | null;
...
export function createEngineeringAnalysisPresentation(analysis: EngineeringAnalysisResult): Readonly<EngineeringAnalysisPresentation> {
  if (analysis.executionStatus === 'BLOCKED') return Object.freeze({
    runId: analysis.runId, engineVersion: analysis.engineVersion, executionStatus: analysis.executionStatus,
    completeness: analysis.completeness, inputSnapshotHash: null, pipeline: null, pressureProfile: null,
    pressureComposition: null, pump: null, results: null, diagnostics: null, report: null,
  });
  return Object.freeze({
    runId: analysis.runId, engineVersion: analysis.engineVersion, executionStatus: analysis.executionStatus,
    completeness: analysis.completeness, inputSnapshotHash: analysis.inputSnapshotHash,
    ...
    pressureComposition: createPressureCompositionPresentation(analysis.simulation.pipeline),
    pump: analysis.simulation.pumpAssessment ? createPumpCapabilityPresentation(analysis.simulation.pumpAssessment) : null,
