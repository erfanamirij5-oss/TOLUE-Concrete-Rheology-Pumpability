...
export function createEngineeringAnalysisPresentation(analysis:EngineeringAnalysisResult):Readonly<EngineeringAnalysisPresentation>{
 if(analysis.executionStatus==='BLOCKED')return Object.freeze({runId:analysis.runId,engineVersion:analysis.engineVersion,executionStatus:analysis.executionStatus,completeness:analysis.completeness,inputSnapshotHash:null,pipeline:null,pressureProfile:null,pressureComposition:null,rheologyCurves:null,pump:null,results:null,diagnostics:null,report:null});
 return Object.freeze({runId:analysis.runId,engineVersion:analysis.engineVersion,executionStatus:analysis.executionStatus,completeness:analysis.completeness,inputSnapshotHash:analysis.inputSnapshotHash,pipeline:createPipelinePresentation(analysis.simulation.pipeline),pressureProfile:createPressureProfilePresentation(analysis.simulation.pressureProfile),pressureComposition:createPressureCompositionPresentation(analysis.simulation.pipeline),rheologyCurves:createRheologyCurvesPresentation(analysis.simulation.rheologyCurves),pump:analysis.simulation.pumpAssessment?createPumpCapabilityPresentation(analysis.simulation.pumpAssessment):null,results:createResultCenterPresentation(analysis.resultCenter,analysis.pumpabilityDecision),diagnostics:createDiagnosticsPresentation(analysis.diagnostics),report:createReportExportPresentation(analysis.reportExport,analysis.pdfExportRequest)});
}
