...
function decisionVisual(data:Readonly<Visualization3DPresentation>|undefined):Readonly<{label:string;detail:string;color:string}> {
  ...
  if(acceptableStatus(decision.status))return{label:'قابل قبول',detail:'مسیر از نظر وضعیت محاسبه‌شده قابل عبور است',color:TOLUE_DESIGN_TOKENS.color.statusNominal};
  ...
  if(decision.status==='FAIL_STABILITY')return{label:'رد · پایداری',detail:'شواهد پایداری وضعیت نامطلوب نشان می‌دهد',color:TOLUE_DESIGN_TOKENS.color.statusWarning};
  if(decision.status==='FAIL_BLOCKAGE')return{label:'رد · ریسک انسداد',detail:'شواهد پروژه ریسک گرفتگی را نشان می‌دهد',color:TOLUE_DESIGN_TOKENS.color.statusCritical};
...
export function renderVisualization3DView(root:HTMLElement,data?:Readonly<Visualization3DPresentation>,options:Readonly<Visualization3DViewOptions>={}):void{
  ...
  const subtitle=document.createElement('small');subtitle.textContent='مختصات فضایی XYZ · خط لوله شفاف · نمایش نتایج هسته مهندسی';Object.assign(subtitle.style,{fontSize:'9px',color:TOLUE_DESIGN_TOKENS.color.textMuted,whiteSpace:'nowrap'});titleWrap.append(title,subtitle);
