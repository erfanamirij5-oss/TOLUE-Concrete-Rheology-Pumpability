...
export function appendStraightSpatialSegmentDraft(
  ...
  if (!Number.isFinite(spec.pipeRadiusM) || spec.pipeRadiusM <= 0) throw new Error('PIPELINE-AUTHORING-RADIUS-001');
  if (input.pipeline.lubricationLayerThicknessM >= spec.pipeRadiusM) throw new Error('PIPELINE-AUTHORING-RADIUS-LAYER-001');
