...
export interface SimulationRunInput { runId:string; engineVersion:string; createdAtIso:string; projectMetadata?:ProjectMetadataInput; materials?:MaterialEngineeringInput[]; pipeline:PipelineAnalysisInput; pumpCapability?:Omit<PumpCapabilityInput,'targetFlowRateM3s'|'requiredPressurePa'|'pipelineCompleteness'>; pumpabilityEvidence?:PumpabilityEvidenceSetInput; provenance?:SimulationInputProvenance; lubricationLayerQualification?:LubricationLayerQualificationInput; assumptions?:string[]; }
...
export function executeSimulationRun(input:SimulationRunInput):SimulationRunResult {
 ...
 if(input.pumpCapability){pumpAssessment=assessPumpCapability({targetFlowRateM3s:input.pipeline.targetFlowRateM3s,requiredPressurePa:pipeline.requiredPressurePa,pipelineCompleteness:pipeline.completeness,capabilityCurve:input.pumpCapability.capabilityCurve,provenance:input.pumpCapability.provenance,...(input.pumpCapability.operatingEnvelope?{operatingEnvelope:input.pumpCapability.operatingEnvelope}:{})}); if(pumpAssessment.status==='INSUFFICIENT_DATA')warnings.push('Pump capability cannot be assessed with the available pressure/flow data.');} else warnings.push('Pump capability data is unavailable; pressure margin was not assessed.');
