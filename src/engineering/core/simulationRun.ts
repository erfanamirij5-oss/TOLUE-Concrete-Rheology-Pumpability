...
import type { PumpabilityRiskScreeningInput } from './pumpabilityRiskScreening';
import { buildBinghamRheologyCurve, type RheologyCurveResult } from './rheologyCurve';
import type { LubricationLayerQualificationInput } from './lubricationLayerQualification';
import type { MaterialEngineeringInput, ProjectMetadataInput } from './projectMaterialInput';
...
export interface SimulationRunInput {
  ...
  materials?:MaterialEngineeringInput[];
  ...
  pumpabilityRiskScreening?:PumpabilityRiskScreeningInput;
  provenance?:SimulationInputProvenance;
  lubricationLayerQualification?:LubricationLayerQualificationInput;
  assumptions?:string[];
}
export interface SimulationRheologyCurves {
  readonly bulk: Readonly<RheologyCurveResult>;
  readonly lubricationLayer: Readonly<RheologyCurveResult>;
  readonly samplingShearRatesSInv: readonly number[];
  readonly method: 'tolue-simulation-rheology-curves-v1';
}
export interface SimulationRunResult {
  ...
  rheologyCurves: Readonly<SimulationRheologyCurves>;
  warnings:string[];
  assumptions:string[];
  methods:string[];
}
...
export function executeSimulationRun(input:SimulationRunInput):SimulationRunResult {
  ...
  if(pipeline.completeness==='incomplete')warnings.push('Pipeline contains pressure contributions that are not computed; required pressure is incomplete.');
  ...
  if(input.pumpCapability){
    pumpAssessment=assessPumpCapability({
      ...
      ...(input.pumpCapability.operatingEnvelope?{operatingEnvelope:input.pumpCapability.operatingEnvelope}:{})
    });
    if(pumpAssessment.status==='INSUFFICIENT_DATA')warnings.push('Pump capability cannot be assessed with the available pressure/flow data.');
  } else warnings.push('Pump capability data is unavailable; pressure margin was not assessed.');
  const status:SimulationRunStatus=pipeline.completeness==='complete'&&pumpAssessment!==null&&pumpAssessment.status!=='INSUFFICIENT_DATA'?'complete':'incomplete';
  const usesProjectCalibratedLocalLoss=pipeline.segments.some(segment=>segment.pressureMethod==='project-calibrated-local-loss');
  return {
    runId:input.runId,
    ...
    methods:[
      ...
      rheologyCurves.method,
      rheologyCurves.bulk.method,
