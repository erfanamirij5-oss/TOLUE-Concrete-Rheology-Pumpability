...
import { evaluateModelReadinessGovernance } from './modelRegistry';

export type ReadinessStatus = 'READY' | 'PRELIMINARY' | 'BLOCKED';
export type ReadinessSeverity = 'info' | 'warning' | 'blocking';

export interface ReadinessFinding { id:string; severity:ReadinessSeverity; field:string; message:string; ruleId:string; }
export interface EngineeringReadinessResult { status:ReadinessStatus; canExecute:boolean; findings:ReadinessFinding[]; method:'tolue-engineering-readiness-gate-v2'; }
...
export function assessEngineeringReadiness(input: SimulationRunInput): EngineeringReadinessResult {
  ...
  const block=(id:string,field:string,message:string,ruleId:string)=>findings.push({id,severity:'blocking',field,message,ruleId});
  const warn=(id:string,field:string,message:string,ruleId:string)=>findings.push({id,severity:'warning',field,message,ruleId});
  const governModel=(modelId:string,field:string,ruleId='RG-MODEL-REGISTRY-001')=>{
    const governance=evaluateModelReadinessGovernance(modelId);
    if(governance.disposition==='BLOCKED') block(`readiness.model.${modelId}.blocked`,field,governance.message,ruleId);
    else if(governance.disposition==='PRELIMINARY') warn(`readiness.model.${modelId}.preliminary`,field,governance.message,ruleId);
  };
  ...
  if((input.assumptions??[]).includes(DRAFT_PLACEHOLDER_ASSUMPTION))block('readiness.draft.placeholderValues','assumptions','Starter draft placeholder values are still active. Replace the starter engineering inputs with deliberate project values and clear the placeholder marker before analysis.','RG-DRAFT-PLACEHOLDER-001');

  const projectMaterials=assessProjectAndMaterialInputs(input.projectMetadata,input.materials);
  projectMaterials.findings.forEach((f,i)=>f.severity==='blocking'?block(`readiness.projectMaterial.${i}`,f.field,f.message,f.ruleId):warn(`readiness.projectMaterial.${i}`,f.field,f.message,f.ruleId));
  ...
  if(!Array.isArray(p.segments)||p.segments.length===0)block('readiness.pipeline.empty','pipeline.segments','At least one pipeline segment is required.','RG-ROUTE-001');
    ...
    for(const segment of p.segments){
      ...
      if(ids.has(segment.id))block(`readiness.segment.duplicate.${segment.id}`,'pipeline.segments',`Duplicate segment ID: ${segment.id}.`,'RG-ROUTE-003');ids.add(segment.id);
      ...
      if(segment.kind==='straight'){
      ...
      }else if(!segment.calibratedLocalLoss)block(`readiness.unsupported.${segment.id}`,`pipeline.segments.${segment.id}`,`Friction model for segment kind '${segment.kind}' is not implemented and no project-calibrated local-loss curve is supplied; complete pressure demand cannot be computed.`,'RG-MODEL-001');
      else{
        usesProjectCalibratedLocalLoss=true;
        try{
          ...
          const entityId=segment.calibratedLocalLoss.provenanceEntityId;const record=input.provenance?.localLossCalibrations?.[entityId];
        ...
        }catch(error){const message=error instanceof Error?error.message:'Invalid project-calibrated local-loss contract.';block(`readiness.localCalibration.invalid.${segment.id}`,`pipeline.segments.${segment.id}.calibratedLocalLoss`,`Invalid project-calibrated local-loss contract: ${message}`,'RG-LOCAL-CAL-002');}
      }
    }
  }
  ...
  if(!input.pumpCapability)block('readiness.pump.missing','pumpCapability','Pump capability data is required for a complete pumpability pressure assessment.','RG-PUMP-001');
    ...
    let previous=-Infinity;for(const point of input.pumpCapability.capabilityCurve){if(!finiteNonNegative(point.flowRateM3s)||!finiteNonNegative(point.availableConcretePressurePa)){block('readiness.pump.curveInvalid','pumpCapability.capabilityCurve','Pump capability points must contain finite non-negative flow and pressure values.','RG-PUMP-003');break;}if(point.flowRateM3s<=previous){block('readiness.pump.curveOrder','pumpCapability.capabilityCurve','Pump capability flow points must be strictly increasing.','RG-PUMP-004');break;}previous=point.flowRateM3s;}
    const first=input.pumpCapability.capabilityCurve[0],last=input.pumpCapability.capabilityCurve[input.pumpCapability.capabilityCurve.length-1];if(first&&last&&Number.isFinite(p.targetFlowRateM3s)&&(p.targetFlowRateM3s<first.flowRateM3s||p.targetFlowRateM3s>last.flowRateM3s))block('readiness.pump.noExtrapolation','pipeline.targetFlowRateM3s','Target flow is outside the supplied pump capability curve; extrapolation is prohibited.','RG-PUMP-005');
    ...
    else{const envelope=qualifyPumpOperatingEnvelope({identity:{manufacturer:input.pumpCapability.operatingEnvelope.manufacturer,model:input.pumpCapability.operatingEnvelope.model,configurationRevision:input.pumpCapability.operatingEnvelope.configurationRevision},source:{documentId:input.pumpCapability.operatingEnvelope.sourceDocumentId,documentRevision:input.pumpCapability.operatingEnvelope.sourceDocumentRevision,...(input.pumpCapability.operatingEnvelope.sourceHash?{sourceHash:input.pumpCapability.operatingEnvelope.sourceHash}:{})},provenance:input.pumpCapability.provenance,capabilityCurve:input.pumpCapability.capabilityCurve});envelope.findings.forEach((f,i)=>{const message=f.replace(/^(BLOCK|PRELIMINARY):/,'').replaceAll('-',' ');f.startsWith('BLOCK:')?block(`readiness.pump.envelope.${i}`,'pumpCapability.operatingEnvelope',message,'RG-PUMP-ENVELOPE-002'):warn(`readiness.pump.envelope.${i}`,'pumpCapability.operatingEnvelope',message,'RG-PUMP-ENVELOPE-003');});}
  }

  if(input.provenance){const evidence=assessInputEvidence(input.provenance);evidence.findings.forEach((f,i)=>f.severity==='blocking'?block(`readiness.provenance.${f.field}.${i}`,`provenance.${f.field}`,f.message,f.ruleId):warn(`readiness.provenance.${f.field}.${i}`,`provenance.${f.field}`,f.message,f.ruleId));}
  ...
  const llQualification=assessLubricationLayerQualification(input.lubricationLayerQualification,input.provenance);llQualification.findings.forEach((f,i)=>f.severity==='blocking'?block(`readiness.lubricationLayerQualification.${i}`,'lubricationLayerQualification',f.message,f.ruleId):warn(`readiness.lubricationLayerQualification.${i}`,'lubricationLayerQualification',f.message,f.ruleId));
  const nonPlaceholderAssumptions=(input.assumptions??[]).filter(a=>a!==DRAFT_PLACEHOLDER_ASSUMPTION);if(nonPlaceholderAssumptions.length>0)warn('readiness.assumptions.present','assumptions','Explicit assumptions are present; if no blocking finding exists, the run is classified PRELIMINARY and assumptions must remain traceable.','RG-PROV-001');
