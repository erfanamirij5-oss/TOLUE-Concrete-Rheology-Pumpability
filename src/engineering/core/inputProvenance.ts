...
export interface SimulationInputProvenance {
  ...
  /**
   * Project-calibrated local-loss evidence indexed by the stable provenance
   * entity ID referenced by the corresponding pipeline segment contract.
   */
  localLossCalibrations?: Record<string, EngineeringInputProvenanceRecord>;
}

export type CoreInputEvidenceField =
  | 'bulkRheology'
  | 'lubricationLayerRheology'
...
export interface InputEvidenceAssessment {
  ...
  method: 'tolue-input-provenance-v3';
}
...
function assessRecord(
  field: InputEvidenceField,
  record: EngineeringInputProvenanceRecord,
  findings: InputEvidenceFinding[],
): void {
  ...
  if (!nonEmpty(evidence.entityId) || !nonEmpty(evidence.generatedByActivityId)) {
    block('PROV-LINEAGE-001', `${field}: entity and generating activity IDs are required.`);
    return;
  }
  ...
  if (activityIds.size !== record.activities.length || record.activities.some(activity => !nonEmpty(activity.id))) {
    block('PROV-LINEAGE-002', `${field}: activity IDs must be non-empty and unique.`);
  }

  const generatingActivity = record.activities.find(activity => activity.id === evidence.generatedByActivityId);
  if (!generatingActivity) {
    block('PROV-LINEAGE-003', `${field}: generatedByActivityId must resolve to a recorded activity.`);
    return;
  }
  ...
  if (agentIds.size !== record.agents.length || record.agents.some(agent => !nonEmpty(agent.id))) {
    block('PROV-LINEAGE-004', `${field}: agent IDs must be non-empty and unique.`);
  }

  for (const activity of record.activities) {
    if (invalidOptionalIso(activity.startedAtIso) || invalidOptionalIso(activity.endedAtIso)) {
      block('PROV-TIME-001', `${field}: activity timestamps must be valid ISO date/time values when supplied.`);
    }
    if (activity.startedAtIso && activity.endedAtIso && Date.parse(activity.startedAtIso) > Date.parse(activity.endedAtIso)) {
      block('PROV-TIME-002', `${field}: activity start time cannot be after end time.`);
    }
    for (const agentId of activity.agentIds ?? []) {
      if (!agentIds.has(agentId)) block('PROV-AGENT-001', `${field}: activity references unknown agent '${agentId}'.`);
    }
    for (const link of activity.calibrationChain ?? []) {
      if (!nonEmpty(link.calibrationId) || !nonEmpty(link.referenceId)) {
        block('PROV-CAL-001', `${field}: calibration links require calibration and reference IDs.`);
      }
      if (invalidOptionalIso(link.calibratedAtIso)) {
        block('PROV-CAL-002', `${field}: calibration timestamps must be valid ISO date/time values when supplied.`);
      }
    }
  }

  if (evidence.entityKind === 'engineering_assumption' || generatingActivity.kind === 'assumption') {
    warn('PROV-ASSUMPTION-001', `${field}: value is an engineering assumption and must remain preliminary.`);
  }
  ...
  if (generatingActivity.kind === 'derivation' && (evidence.sourceEntityIds ?? []).length === 0) {
    warn('PROV-DERIVE-001', `${field}: derived result should identify its source entities.`);
  }

  if (evidence.entityKind === 'manufacturer_data' && !nonEmpty(evidence.sourceDocumentId)) {
    warn('PROV-MFR-001', `${field}: manufacturer data should identify the source document and version.`);
  }
}
...
export function assessInputEvidence(provenance: SimulationInputProvenance): InputEvidenceAssessment {
  ...
  const coreFields: CoreInputEvidenceField[] = [
    'bulkRheology',
    'lubricationLayerRheology',
    'lubricationLayerThickness',
    'pumpCapability',
  ...
  for (const field of coreFields) assessRecord(field, provenance[field], findings);
  ...
  for (const [entityId, record] of Object.entries(provenance.localLossCalibrations ?? {})) {
    ...
    if (!nonEmpty(entityId)) {
      findings.push({ field, severity: 'blocking', ruleId: 'PROV-LOCAL-001', message: `${field}: local-loss provenance map key must be non-empty.` });
      continue;
    }
    if (record.evidence.entityId !== entityId) {
      findings.push({ field, severity: 'blocking', ruleId: 'PROV-LOCAL-002', message: `${field}: map key must exactly match evidence.entityId.` });
    }
