...
| PRESSURE-LOCAL-CAL-001 | Pump hydraulics | implemented | Project-calibrated local component pressure-loss curve for elbow/reducer/hose/valve/boom/other using traceable project data | Exact points and linear interpolation inside the supplied calibration curve only; extrapolation is prohibited; output is project-specific and must never be presented as a universal component law; pipeline integration requires provenance/readiness/result-semantic synchronization |
