...
| PRESSURE-ELBOW-002 | Pump hydraulics | numerically_verified | Gao et al. 2024 empirical bend correction-factor model; isolated lambda evaluator verified in published regression domain | Full bend-pressure integration remains blocked because Gao lambda is calibrated against a different native one-dimensional base model; direct multiplication by TOLUE two-fluid pressure and `(lambda-1)` transfer are prohibited until native source-model reproduction and end-to-end pressure verification are complete |
